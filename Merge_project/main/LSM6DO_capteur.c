 ...
 while (1) {
	...
	if (flag)
	{
      ...
      if (flag) {
        ...
        xQueueSend(xQueue, &temperature_degC, 150); // Envoi de la valeur dans la file d'attente
...
void whoami_LSM6DO_task()
{
    ...
    //while (1) {
       ...
       //get_accel_task();
       //xTaskCreate(get_accel_task, "lsm6dso_get_Acc_task", 10000, NULL, 1, &acc_task_Handle);
       //get_accel_task();
       xTaskCreate(get_accel_task, "lsm6dso_get_Acc_task", 1024 * 2, (void *)0, 3, NULL);
       //get_accel_task();
